    ...
    return(
        <section className='w-full flex flex-wrap-reverse lg:flex-nowrap justify-center lg:justify-between items-center gap-8'>
            <div className='flex flex-col gap-6 max-w-[550px] items-center lg:items-start'>
                <Image src={BannerFlower} alt='' draggable="false"/>
                <h1 className={`${spicyRice.className} text-center lg:text-left text-7xl text-vintage-green max-w-[400px] uppercase`}>Eternize a voz de quem você ama</h1>
                <p className={`${inter.className} text-center lg:text-left text-2xl text-neutral-900`}>A forma simples e autêntica de guardar as mensagens de seus convidados.</p>
                <Link href="#contact"><button><p className={`${inter.className} text-neutral-900 text-xl font-black uppercase bg-vintage-yellow px-8 py-4  rounded-full border border-neutral-900 hover:text-neutral-900/80 transition-colors`}>Consulte agenda</p></button></Link>
            </div>
